# FI V26 Final Submission Package

This directory is the final Frontiers in Immunology submission package generated from the checked V25 figure package on 2026-06-26.

## Upload-facing contents

- Main manuscript DOCX: `Frontiers_Immunology_V26_FI_FINAL_SUBMISSION_MAIN_TEXT.docx`
- Supplementary materials index DOCX: `Frontiers_Immunology_V26_FI_FINAL_SUPPLEMENTARY_MATERIALS_INDEX_ONLY.docx`
- Main figures: `Figure_01` to `Figure_05` in SVG, PDF, TIFF, and PNG
- Formal supplementary figure: `Supplementary_Figure_S1` in SVG, PDF, TIFF, and PNG
- Supplementary tables: `Supplementary_Tables_S1_to_S22_CSV.zip`

## Authorship

The final author list is Xinzhuo Li and Yi Gong. Yi Gong is the corresponding author.

## Repository and archive

- GitHub repository: https://github.com/LiXinzhuo0425/sepsis-multiscale-immune-decoupling
- Zenodo version DOI pending for v0.1.4; prior archived version DOI: https://doi.org/10.5281/zenodo.20794161
- Previous Zenodo version DOI: https://doi.org/10.5281/zenodo.20794161

## QA

Structural and upload checks are recorded in `04_QA_and_manifests/FI_V26_final_submission_QA_20260626.json`.
